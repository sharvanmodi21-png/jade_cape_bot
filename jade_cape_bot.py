...
class JadeCapeBot:
    ...
    def _scan_raid(self, df: pd.DataFrame, zones: dict, lookback: int):
        """Scan the last `lookback` candles of df for a sweep of any zone."""
    ...
    def detect_liquidity_raid(self, df_15m: pd.DataFrame, zones: dict, df_5m: pd.DataFrame = None):
        """Return (raid_detected, level_name, sweep_price).
        Checks the 15m timeframe first, then the faster 5m timeframe so raids
        are caught within a 5-minute candle instead of waiting for a 15m close.
        """
        # 15m: last 4 candles (~1 hour)
        raid, level, price = self._scan_raid(df_15m, zones, lookback=4)
        if raid:
            return True, level, price
        # 5m: last 6 candles (~30 min) for faster reaction
        if df_5m is not None:
            raid, level, price = self._scan_raid(df_5m, zones, lookback=6)
            if raid:
                return True, level, price
        return False, None, 0.0
    ...
    def check_and_trade(self):
        ...
        try:
            ...
            raid, raid_level, raid_price = self.detect_liquidity_raid(df_15m, zones, df_5m)
            if not raid:
